...
def get_stock_values(data):
    """
    Retrieves the column headings from the stock worksheet and combines
    them with the calculated stock data into a dictionary.
    """
    # Retrieve the first row (headings) from the 'stock' worksheet.
    # This is done by getting all values and taking the item at index 0.
    headings = SHEET.worksheet("stock").get_all_values()[0]

    # Create a dictionary using a dictionary comprehension, pairing
    # each heading (key) with its corresponding calculated stock data (value).
    stock_dict = {
        key: value for key, value in zip(headings, data)
    }
...
# Assign it the return value from calling the function get_stock_values
